...
# Fix: Check if AssistantVoice is loaded properly, provide default if not
if not AssistantVoice:
    AssistantVoice = "en-US-AriaNeural"  # Default voice if not found in .env file
    print("Warning: AssistantVoice not found in .env file, using default voice")
        ...
        try:
            ...
            while pygame.mixer.music.get_busy():
                if func() == False: # Check if the external function returns False
                    break
                pygame.time.Clock().tick(10) # Limit the loop to 10 ticks per sec
            ...
            break  # Break the while loop to prevent infinite error loop
